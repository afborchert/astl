...
Astl program sources are to be encoded in UTF-8\index{UTF-8}. While most Astl
token including identifiers consist of ASCII\index{ASCII} characters only,
...
multiply of 8 plus 1. Unicode\index{Unicode} codepoints are counted as one character
independent from the number of bytes required to encode them in UTF-8\index{UTF-8}.
