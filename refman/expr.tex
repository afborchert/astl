...
\section{Abstract syntax tree constructors}
...
subnodes whose elements are inserted at the corresponding place. The
following example constructs abstract syntax trees that correspond to $3
+ 4 * 5$ and $2 * (3 + 4 * 5)$:

\begin{lstlisting}
   var node = <("+" { token("3") } ("*" { token("4") } { token("5") }))>;
   var expr = <("*" { token("2") } node)>;
\end{lstlisting}
