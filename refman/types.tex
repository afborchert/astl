...
type of values: \keyword{null}, Boolean values, integers, strings,
...
on the GNU Multiple Precision Library\index{GNU Multiple Precision
Library}. Please note that floating point\index{floating point}
...
are bound to variables (\ident{id}, \ident{declarator} and \ident{block}
...
designates the entry of the dictionary \ident{block} selected
by the key \ident{vars}. Whenever a key is constant and an identifier,
...
Function values consist of a function body and a closure\index{closure}.
...
Please note that there exist two kinds of leafs\index{leaf}, i.e. token
...
\noindent
In the following attribution rule, \ident{block} is bound
to a operator node with the operator \lstinline!"compound_statement"!.
The assignments set the attributes \ident{vars}, \ident{level}, and
\ident{up} of \ident{block} to various values.
...
\section{Control flow graph nodes}\index{control flow graph}
...
to a node of an abstract syntax tree, the \ident{astnode} field
allows to access it. In addition, a \keyword{foreach} loop
may be used to iterate through all the successor nodes.
In case of labeled edges, the branches can be examined in
the \ident{branch} dictionary.

The set of graphs $G$ is represented by the predefined
dictionary \ident{graph}\index{graph}. Per convention
...
\ident{len}\index{len} standard function). Functions are invoked with
