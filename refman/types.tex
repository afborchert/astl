...
node is bound to \ident{block} due to the \keyword{as} clause.
...
\ident{vars}, \ident{level}, and \ident{up} of \ident{block}:
...
to a node of an abstract syntax tree, the \ident{astnode} field allows
to access it. In addition, a \keyword{foreach} loop may be used to
iterate through all the successor nodes. In case of labeled edges, the
branches can be examined in the \ident{branch} dictionary.
